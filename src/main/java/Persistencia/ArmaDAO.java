...
public class ArmaDAO {
	...
	public Arma obtenerArmaPorId(String idArma) {
		...
		// 1. Le decimos a Mongo: "Busca un documento donde el campo 'idArma' sea igual
		// al que me pasan por parámetro y dame el primero que encuentres"
		Document doc = coleccion.find(Filters.eq("idArma", idArma)).first();

		// 2. Control de errores: Si la base de datos no encuentra el arma, devolvemos
		// null
		if (doc == null) {
			return null;
		}
		...
		// 4. Vamos sacando la información del documento (JSON) y la metemos en nuestro
		// objeto Java
	...
	// Método para obtener una lista con todas las armas (Ideal para llenar ComboBox
	// en JavaFX)
